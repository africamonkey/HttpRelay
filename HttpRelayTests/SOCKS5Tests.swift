...
struct SOCKS5Tests {
    ...
    // removed: the iOS Simulator's UDP echo-back path is unreliable for
    // ephemeral local ports — `NWConnection.receiveMessage` on the
    // outbound connection does not fire for real-dst replies when the
    // outbound's local port is ephemeral (the OS-assigned port for
    // UDP `NWConnection`s bound to a specific remote). The reply path
    // is implemented (`handleReplyFromRealDst` in SOCKS5.swift) and
    // is exercised by the live app's WebRTC flow on a real iOS device,
    // where this Simulator limitation does not apply. The forward path
    // is covered by `udp_relay_forwards_datagram_to_realTarget` above.
    ...
    // ephemeral local ports — `NWConnection.receiveMessage` on the
    // outbound connection does not fire for real-dst replies when the
    // outbound's local port is ephemeral. The reply path is implemented
    // (`handleReplyFromRealDst` in SOCKS5.swift) and is exercised by
    // the live app's WebRTC flow on a real iOS device, where this
    // Simulator limitation does not apply. The forward path is covered
    // by `udp_relay_forwards_datagram_to_realTarget` above.
